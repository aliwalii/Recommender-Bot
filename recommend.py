...
class Recommend(commands.Cog, name="recommend"):
    ...
    async def update_rating(self, discord_user, partial_movie_title: str, new_rating: float):
        ...
        with open(self.data_file, 'w') as file:
            ...
            for line in lines:
                if line.startswith(f"{user_id}\t{movie_id}\t"):
                    file.write(f"{user_id}\t{movie_id}\t{new_rating}\t0\n")
                    rating_updated = True
                else:
                    file.write(line)
        ...
        return True, f"Rating updated for Discord user '{discord_username}' on movie '{movie_title}'."
    ...
    @commands.hybrid_command(
        name="update_rating",
        description="Update a movie rating for a Discord user.",
    )
    async def update_rating_command(self, ctx: commands.Context, movie_title: str, new_rating: float):
        success, message = await self.update_rating(ctx.author, movie_title, new_rating)
        await ctx.send(message)
    ...
    async def recommend(self, ctx: commands.Context, *, partial_movie_name: str):
        ...
        rating_rounded = round(prediction.est)
        stars = '⦿' * rating_rounded + '⦾' * (5 - rating_rounded)

        embed = discord.Embed(
            title=f"Closest match: '{movie_name}'",
            description=f"Prediction for User '{discord_username}' on Movie '{movie_name}':\nRating Prediction:  {stars} {"|"} {round(prediction.est, 2)}",
            color=0x57F287,
        )
